		...
		function pages:CreateDropdown(name, default, options, cb)
			...
			--[[local function toggle()
				...
				TweenService:Create(
					Dropdown,
					TweenInfo.new(.5), {
						Size = UDim2.new(1, 0, 0, (opened and 30) or 0)
					}
				):Play()
			...
			local function toggle()
				...
				TweenService:Create(Arrow, TweenInfo.new(.5), {Rotation = (opened and 90) or 0}):Play()
				TweenService:Create(ChildrenFrame, TweenInfo.new(.5), {Size = UDim2.new(1, 0, 0, (opened and 30 * (#ChildrenFrame:GetChildren() - 1)) or 0)}):Play()
